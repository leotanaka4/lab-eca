...
function import_signals(frequencies, num_samples, c1_folder)%, c2_folder)
    ...
    % Criar uma nova figura para os subplots
    figure;
    
    % Determinar o número total de frequências
    num_frequencies = length(frequencies);
    
    % Loop através de todas as frequências
    for i = 1:num_frequencies
        ...
        % Criar subplots
        subplot(ceil(num_frequencies / 5), 5, i); % Configurar o subplot
        hold on;
        ...
        legend('u(t)',  'y(t)',  'a_0_u + a_1_u sin(\omega t)',  'a_0_y + a_1_y sin(\omega t)', 'a_0_y','a_0_u', ...
            'Location', 'Best', 'FontSize', 6); % Ajustar o tamanho da fonte da legenda
        title(sprintf('%.2f Hz', str2double(strrep(freq_str, '_', '.'))));
        xlabel('Tempo (s)');
        ylabel('Amplitude (V)');
        grid on;
        hold off;        
    ...
    sgtitle('Comparação de Sinais e Aproximações para Diferentes Frequências');  % Título global
end
